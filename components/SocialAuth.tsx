...
const PROVIDERS: { id: Provider; label: string; icon: React.ReactNode; bg: string; border: string; color: string }[] = [
  {
    id: 'github',
    label: 'GitHub',
...
export default function SocialAuth({ mode = 'login' }: { mode?: 'login' | 'signup' }) {
  ...
  async function handleOAuth(provider: Provider) {
    ...
    // Page will redirect — loading state resolves via redirect
  }
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <span style={{ color: '#2a3347', fontSize: 12, fontWeight: 500, whiteSpace: 'nowrap' }}>
        ...
        {PROVIDERS.map(p => (
          <button
            ...
            style={{
              ...
              background: loading === p.id ? 'rgba(255,255,255,0.02)' : p.bg,
              border: `1px solid ${p.border}`,
              borderRadius: 10, padding: '11px 20px',
              color: p.color, fontSize: 14, fontWeight: 500,
            ...
            onMouseEnter={e => { if (!loading) (e.currentTarget as HTMLButtonElement).style.background = 'rgba(255,255,255,0.07)' }}
            onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.background = loading === p.id ? 'rgba(255,255,255,0.02)' : p.bg }}
          >
            {loading === p.id
            ...
            {loading === p.id ? `Connecting to ${p.label}…` : `Continue with ${p.label}`}
          </button>
        ))}
      </div>
